...
class HomeExpertCategoryController extends Controller
{
    ...
    public function store(Request $request){
        $request->validate([
            'title' => 'required|max:255|string|unique:home_expert_categories',
        ...
        if(!empty($request->image)){
            ...
            if(strtoupper($extension)=='SVG' || strtoupper($extension)=='WEBP' || strtoupper($extension)=='AVIF'){
                ...
                $FileName = autoheight('uploads/homecategory/',90,$request->image);
            }
        }
    ...
    public function update(Request $request){
        ...
        if(!empty($r->image)){
            ...
            if(strtoupper($extension)=='SVG' || strtoupper($extension)=='WEBP' || strtoupper($extension)=='AVIF'){
                $FileName = directFile('uploads/homecategory/',$r->image);
            }else{
                $FileName = autoheight('uploads/homecategory/',90,$r->image);
            }
        }

                ...
                <div class="col-lg-9 col-md-10">
                    ...
                    <div class="row align-items-end">
                        <div class="col-12 text-center">
                            <h1 class="mt-2 h6 text-secondary mb-3">Steps 5 out of 5</h1>
                            <h2 class="mt-2 h3 thm mb-3">How did you hear about us?</h2>
                        </div>
                    </div>
                    <form action="{{route('user.saveusersetp5')}}" method="post" class="card card-body formbox">
                        @csrf
                        <ul class="ListBox">
                            @foreach ($lists as $item)
                            <li class="form-check">
                                <input class="form-check-input" type="checkbox" value="{{$item->id ?? 0}}" name="hear_about_us[]" id="role{{$loop->iteration}}">
                                <label class="form-check-label" for="role{{$loop->iteration}}">{{$item->title ?? ''}}</label>
                            </li>
                            @endforeach                           
                        </ul>
                        @error('hear_about_us')<small class="error">{{$message}}</small>@enderror
                    ...
                    <a href="{{route('user.userregister4')}}" class="btn formbtn btn-sm btn-thm3"><i class="fal fa-arrow-left m-0 me-2"></i> Back</a>
                </div>
            </div>
        </div>
        <div class="bg-img">
...
@endsection
@push('css')
<title>Step 5 for User Sign Up : {{project()}}</title>

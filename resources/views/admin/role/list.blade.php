                                                ...
                                                <td class="pd-r-0-force tx-center">
                                                    ...
                                                    <div class="dropdown TAction show">
                                                        ...
                                                        <ul class="dropdown-menu" x-placement="bottom-end">
                                                            ...
                                                            <li><a href="{{route('admin.role.remove',['id'=>$list->id])}}" class="text-danger"
...
<script>
    ...
    $('.sequenceform').on('click',function(){ $('.table-wrapper').attr('action',@json(route('admin.role.sequence'))); $('.table-wrapper').submit(); });
    $('.bulkremoveform').on('click',function(){ $('.table-wrapper').attr('action',@json(route('admin.role.bulkremove'))); $('.table-wrapper').submit(); });
    function changestatus(id){
        let status = 0;
        if($('#flexSwitchCheckDefault'+id).prop('checked')==true){
            status = 1;
        }
        let url = @json(route('admin.role.status'));
        databasestatuschange(url,status,id);
    }
    $('[data-bs-id]').on('click',function(){
        let id = $(this).attr('data-bs-id');
        $('.offcanvas-title').text('Edit Role');
        loadingbox();
        $('.offcanvas-body').load(@json(route('admin.role.edit'))+'?id='+id);
    });
</script>
@endpush
@push('css')

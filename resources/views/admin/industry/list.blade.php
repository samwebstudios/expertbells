                ...
                <div class="col-sm-5">
                    ...
                    <div class="card shadow-base bd-0">
                        ...
                        <div class="card-body justify-content-between align-items-center">
                            ...
                            <form method="post" action="{{ route('admin.industry.save') }}"
                                                ...
                                                <td class="pd-r-0-force tx-center">
                                                    ...
                                                    <div class="dropdown TAction show">
                                                        ...
                                                        <ul class="dropdown-menu" x-placement="bottom-end">
                                                            ...
                                                            <li><a href="{{route('admin.industry.remove',['id'=>$list->id])}}" class="text-danger"
...
<script>
    ...
    $('.sequenceform').on('click',function(){ $('.table-wrapper').attr('action',@json(route('admin.industry.sequence'))); $('.table-wrapper').submit(); });
    $('.bulkremoveform').on('click',function(){ $('.table-wrapper').attr('action',@json(route('admin.industry.bulkremove'))); $('.table-wrapper').submit(); });
    function changestatus(id){
        let status = 0;
        if($('#flexSwitchCheckDefault'+id).prop('checked')==true){
            status = 1;
        }
        let url = @json(route('admin.industry.status'));
        databasestatuschange(url,status,id);
    }
    $('[data-bs-id]').on('click',function(){
        let id = $(this).attr('data-bs-id');
        $('.offcanvas-title').text('Edit Industry');
        loadingbox();
        $('.offcanvas-body').load(@json(route('admin.industry.edit'))+'?id='+id);
    });
</script>
@endpush
@push('css')
<title>Industry Management : {{project()}}</title>

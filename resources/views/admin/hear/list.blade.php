        ...
        <div class="br-pageheader pd-y-15 pd-l-20">
            ...
            <div class="col-md-6">
                ...
                <nav class="breadcrumb pd-0 mg-0 tx-12">
                    ...
                    <span class="breadcrumb-item active">Hear About Us</span>
                ...
                <div class="col-sm-5">
                    ...
                    <div class="card shadow-base bd-0">
                        ...
                        <div class="card-body justify-content-between align-items-center">
                            ...
                            <form method="post" action="{{ route('admin.hearabout.save') }}"
                                                ...
                                                <td class="pd-r-0-force tx-center">
                                                    ...
                                                    <div class="dropdown TAction show">
                                                        ...
                                                        <ul class="dropdown-menu" x-placement="bottom-end">
                                                            ...
                                                            <li><a href="{{route('admin.hearabout.remove',['id'=>$list->id])}}" class="text-danger"
...
<script>
    ...
    $('.sequenceform').on('click',function(){ $('.table-wrapper').attr('action',@json(route('admin.hearabout.sequence'))); $('.table-wrapper').submit(); });
    $('.bulkremoveform').on('click',function(){ $('.table-wrapper').attr('action',@json(route('admin.hearabout.bulkremove'))); $('.table-wrapper').submit(); });
    function changestatus(id){
        let status = 0;
        if($('#flexSwitchCheckDefault'+id).prop('checked')==true){
            status = 1;
        }
        let url = @json(route('admin.hearabout.status'));
        databasestatuschange(url,status,id);
    }
    $('[data-bs-id]').on('click',function(){
        let id = $(this).attr('data-bs-id');
        $('.offcanvas-title').text('Edit Hear About');
        loadingbox();
        $('.offcanvas-body').load(@json(route('admin.hearabout.edit'))+'?id='+id);
    });
</script>
@endpush
@push('css')
<title>Hear About Us : {{project()}}</title>

        ...
        <div class="br-pageheader pd-y-15 pd-l-20">
            ...
            <div class="col-md-6">
                ...
                <nav class="breadcrumb pd-0 mg-0 tx-12">
                    ...
                    <span class="breadcrumb-item active">Working As</span>
                ...
                <div class="col-sm-5">
                    ...
                    <div class="card shadow-base bd-0">
                        ...
                        <div class="card-body justify-content-between align-items-center">
                            ...
                            <form method="post" action="{{ route('admin.working.save') }}"
                                                ...
                                                <td class="pd-r-0-force tx-center">
                                                    ...
                                                    <div class="dropdown TAction show">
                                                        ...
                                                        <ul class="dropdown-menu" x-placement="bottom-end">
                                                            ...
                                                            <li><a href="{{route('admin.working.remove',['id'=>$list->id])}}" class="text-danger"
...
<script>
    ...
    $('.sequenceform').on('click',function(){ $('.table-wrapper').attr('action',@json(route('admin.working.sequence'))); $('.table-wrapper').submit(); });
    $('.bulkremoveform').on('click',function(){ $('.table-wrapper').attr('action',@json(route('admin.working.bulkremove'))); $('.table-wrapper').submit(); });
    function changestatus(id){
        let status = 0;
        if($('#flexSwitchCheckDefault'+id).prop('checked')==true){
            status = 1;
        }
        let url = @json(route('admin.working.status'));
        databasestatuschange(url,status,id);
    }
    $('[data-bs-id]').on('click',function(){
        let id = $(this).attr('data-bs-id');
        $('.offcanvas-title').text('Edit Working');
        loadingbox();
        $('.offcanvas-body').load(@json(route('admin.working.edit'))+'?id='+id);
    });
</script>
@endpush
@push('css')
<title>Working As Management : {{project()}}</title>

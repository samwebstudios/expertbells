        ...
        <div class="br-pageheader pd-y-15 pd-l-20">
            <div class="col-md-6">
                <nav class="breadcrumb pd-0 mg-0 tx-12">
                    ...
                    <span class="breadcrumb-item active">Featured In</span>
                </nav>
            </div>
            <div class="col-md-6">
                <div class="text-right">
                    <a href="{{route('admin.featuredcms')}}" target="_blank" class="btn btn-dark">
                        Section Heading
                    </a>
                </div>
            </div>
                ...
                <div class="col-sm-5">
                    <div class="card shadow-base bd-0">
                        ...
                        <div class="card-body justify-content-between align-items-center">
                            <form method="post" action="{{ route('admin.featured.save') }}"
                                enctype="multipart/form-data">
                                @csrf                                
                                <div class="row mg-b-25">
                                    <div class="col-lg-12">
                                        <div class="form-group">
                                            <label>Alt / Title <span class="error">*</span></label>
                                    ...
                                    <div class="col-lg-8">
                                        <div class="form-group">
                                            <small class="p-0">Image Size is 220px * 90px</small>                              
                                                ...
                                                <td class="pd-r-0-force tx-center">
                                                    ...
                                                    <div class="dropdown TAction show">
                                                        ...
                                                        <ul class="dropdown-menu" x-placement="bottom-end">
                                                            ...
                                                            <li><a href="{{route('admin.featured.remove',['id'=>$list->id])}}" class="text-danger"
...
<script>
    ...
    $('.sequenceform').on('click',function(){ $('.table-wrapper').attr('action',@json(route('admin.featured.sequence'))); $('.table-wrapper').submit(); });
    $('.bulkremoveform').on('click',function(){ $('.table-wrapper').attr('action',@json(route('admin.featured.bulkremove'))); $('.table-wrapper').submit(); });
    function changestatus(id){
        let status = 0;
        if($('#flexSwitchCheckDefault'+id).prop('checked')==true){
            status = 1;
        }
        let url = @json(route('admin.featured.status'));
        databasestatuschange(url,status,id);
    }
    $('[data-bs-id]').on('click',function(){
        let id = $(this).attr('data-bs-id');
        $('.offcanvas-title').text('Edit Featured In');
        loadingbox();
        $('.offcanvas-body').load(@json(route('admin.featured.edit'))+'?id='+id);
    });
    imgInp.onchange = evt => {
...
@endpush
@push('css')
<title>Featured In : {{project()}}</title>

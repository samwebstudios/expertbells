                ...
                <div class="col-sm-5">
                    ...
                    <div class="card shadow-base bd-0">
                        ...
                        <div class="card-body justify-content-between align-items-center">
                            ...
                            <form method="post" action="{{ route('admin.savequalifications') }}"
                                                ...
                                                <td class="pd-r-0-force tx-center">
                                                    ...
                                                    <div class="dropdown TAction show">
                                                        ...
                                                        <ul class="dropdown-menu" x-placement="bottom-end">
                                                            ...
                                                            <li><a href="{{route('admin.removequalifications',['id'=>$list->id])}}" class="text-danger"
...
<script>
    ...
    $('.sequenceform').on('click',function(){ $('.table-wrapper').attr('action',@json(route('admin.qualificationsequence'))); $('.table-wrapper').submit(); });
    $('.bulkremoveform').on('click',function(){ $('.table-wrapper').attr('action',@json(route('admin.bulkremovequalification'))); $('.table-wrapper').submit(); });
    function changestatus(id){
        let status = 0;
        if($('#flexSwitchCheckDefault'+id).prop('checked')==true){
            status = 1;
        }
        let url = @json(route('admin.changequalificationstatus'));
        databasestatuschange(url,status,id);
    }
    $('[data-bs-id]').on('click',function(){
        let id = $(this).attr('data-bs-id');
        $('.offcanvas-title').text('Edit Qualification');
        loadingbox();
        $('.offcanvas-body').load(@json(route('admin.editqualification'))+'?id='+id);
    });
</script>
@endpush
@push('css')
<title>Qualification Management : {{project()}}</title>
